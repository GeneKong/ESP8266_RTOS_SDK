...
from .tools.eclipse import EclipseMakeGcc
from .tools.gnu_mcu_eclipse import EclipseGnuMCU
from .tools.st_cube_eclipse import EclipseStCube
...
class ToolsSupported:
    ...
    TOOLS_ALIAS = {
        ...
        'make_armcc':    'armcc',
        'make_gcc':      'gcc',
        'eclipse':       'eclipse_make_gcc',
        'gnu_mcu':       'gnu_mcu_eclipse',
        'st_cube':       'st_cube_eclipse',
    ...
    TOOLS_DICT = {
        ...
        'gcc':                  MakefileGcc,
        'armcc':                MakefileArmcc,
        'eclipse_make_gcc':     EclipseMakeGcc,
        'gnu_mcu_eclipse':      EclipseGnuMCU,
        'st_cube_eclipse':      EclipseStCube,
    ...
    TOOLCHAINS = list(set([v.get_toolchain() for k, v in list(TOOLS_DICT.items()) if v.get_toolchain() is not None]))
    TOOLS = list(set([v for k, v in list(TOOLS_DICT.items()) if v is not None]))

    def _get_tool_name(self, tool):
        if tool in list(self.TOOLS_ALIAS.keys()):
