...
class Generator:
    ...
    def _generate_subproj(self, project):
        ...
        for sproj in list(project.sub_projects.values()):
    ...
    def generate(self, name='', tool='gnu_mcu_eclipse'):
        ...
        if name != '':
            ...
            if 'projects' in self.projects_dict:
                if name in list(self.projects_dict['projects'].keys()):
                    found = True
            ...
            if 'projects' in self.projects_dict:
                found = True
                for name, records in sorted(list(self.projects_dict['projects'].items()),
                                            key=lambda x: x[0]):
                    if not records:
                        records = {}
                    self.push_properties()
