...
def create_yaml(directory, project_name, board,output_dir):
    ...
    for section, extensions in list(common_section.items()):
    ...
    for tool, linker in list(linkers.items()):
        if len(linker) > 1:
            ...
            while int(answer) not in list(range(0,len(linker))):
                answer = input('Answer not in list. Which file do you want? ')
