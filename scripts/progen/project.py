...
class Project:
    ...
    def update_from_required(self, subproj, ptype):
        ...
        if "files" in src_project:
            ...
            for key, value in list(subproj.project["files"]["includes"].items()):
            ...
            if ptype == "src":
                for key, value in list(subproj.project["files"]["sources"].items()):
    ...
    def _dict_elim_none(dic_to_clean):
        dic = dic_to_clean
        try:
            for k, v in list(dic_to_clean.items()):
                if type(v) is list:
    ...
    def _process_include_files(self, files, use_group_name = 'default', add_basepath = True):
        ...
        if type(files) in [dict,  OrderedDict]:
            for group_name, include_files in list(files.items()):
    ...
    def _process_source_files(self, files, use_group_name='default', add_basepath = True):
        use_sources = []
        if type(files) == dict:
            for group_name, sources in list(files.items()):
    ...
    def _copy_sources_to_generated_destination(self):
        ...
        files = []
        for key in list(FILES_EXTENSIONS.keys()):
            if key in self.export:
                if type(self.export[key]) is dict:
                    for k,v in list(self.export[key].items()):
                        files.extend(v)
