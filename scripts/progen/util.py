...
FILE_MAP = {v:k for k,values in list(FILES_EXTENSIONS.items()) for v in values}
...
def fixed_path_2_unix_style(item):
    nitem = None
    if type(item) == list:
        nitem = []
        for path in item:
        ...
        for k,v in list(item.items()):
            nitem[k] = fixed_path_2_unix_style(v)
        return nitem
    else:
        return item.replace('\\', '/')
    ...
    for key in extensions:
        if key in project_data:
            if type(project_data[key]) is dict or type(project_data[key]) is OrderedDict:
                for k,v in list(project_data[key].items()):
