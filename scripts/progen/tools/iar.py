...
class IAREmbeddedWorkbenchProject:
    ...
    def _ewp_flags_set(self, ewp_dic_subset, project_dic, flag_type, flag_dic):
        ...
        try:
            if flag_type in list(project_dic['misc'].keys()):
                # enable commands
    ...
    def _ewp_files_set(self, ewp_dic, project_dic):
        ...
        i = 0
        for group_name, files in list(project_dic['groups'].items()):
...
class IAREmbeddedWorkbench(Tool, Builder, Exporter, IAREmbeddedWorkbenchProject):
    ...
    def _fix_paths(self, data):
        ...
        for attribute in SOURCE_KEYS:
            for k, v in list(data[attribute].items()):
                if k not in data['groups']:
                    data['groups'][k] = []
                data['groups'][k].extend([join('$PROJ_DIR$', file) for file in v])
        for k,v in list(data['include_files'].items()):
            if k not in data['groups']:
                data['groups'][k] = []
            data['groups'][k].extend([join('$PROJ_DIR$', file) for file in v])

        # sort groups
        data['groups'] = OrderedDict(sorted(list(data['groups'].items()), key=lambda t: t[0]))
    ...
    def _export_single_project(self):
        ...
        elif 'iar' in list(self.env_settings.templates.keys()):
            template_ewp = False
            template_ewd = False
        ...
        if self.workspace['singular']:
            ...
            eww_dic = {'workspace': {'project': {'path': ''}, 'batchBuild': None}}
            # set eww
