...
class Exporter(object):
    ...
    def _expand_data(self, old_data, new_data, group):
        ...
        for file in old_data:
            if file:
                ...
                if extension in list(self.file_types.keys()):
    ...
    def _iterate(self, data, expanded_data):
        ...
        for attribute in SOURCE_KEYS:
            for k, v in list(data[attribute].items()):
                if k == None:
                    ...
                    group = k
                if group in list(data[attribute].keys()):
        ...
        # sort groups
        expanded_data['groups'] = OrderedDict(sorted(list(expanded_data['groups'].items()), key=lambda t: t[0]))
